...
- There are 1033 observations and 15 columns in the combined dataset
  (*combined_tw_df*), including 651 observations from the Mr. Trash
  Wheel dataset (*mr_tw_df*), 119 observations from the Professor Trash
  Wheel dataset (*prof_tw_df*), and 263 observations from the Gwynnda
  Trash Wheel dataset (*gwyn_tw_df*).
...
# use 'select()' to remove region_type (have that info when we rename region_name) and state (repeated variable as state_name)
# remove " County" in county_name to be consistent with zip_code_df
...
zori_df = 
  ...
  rename_with(~ sub("^x", "", .x)) |> 
  select(metro, state_name, city_name, county_name_zori, zip_code, region_id, size_rank, everything())
```
